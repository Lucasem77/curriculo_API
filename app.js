...
app.put('/curriculo/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `UPDATE curriculo SET nome = $1, number = $2, phone_number = $3, cep = $4, email = $5, complement = $6, district = $7, linkedin = $8, nationality = $9 
...
app.delete('/curriculo/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query('DELETE FROM curriculo WHERE id = $1 RETURNING *', [id]);
